...
typedef struct {
    ...
    int prompt_len;                 // current prompt length
    char cmdline [_COMMAND_LINE_LEN];  // cmdline buffer
	int cmdlen;                        // last position in command line
	int cursor;                        // input cursor
    int (*execute) (void *extra,int argc, const char * const * argv );            // ptr to 'execute' callback
    char ** (*get_completion) (void *extra,int argc, const char * const * argv ); // ptr to 'completion' callback
    void (*print) (void *extra, const char *);                                     // ptr to 'print' callback
